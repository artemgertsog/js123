...
function showNetworkNotification(type, message) {
  if (!notificationElement) {
    ...
    if (app.firstChild) {
      app.insertBefore(notificationElement, app.firstChild);
    } else {
  ...
  if (type === 'error') {
    ...
    notificationElement.classList.remove('slow');
  } else if (type === 'slow') {
    notificationElement.classList.add('slow');
...
function hideNetworkNotification() {
  if (notificationElement) {
    notificationElement.classList.remove('show', 'error', 'slow');
...
function checkNetworkConnection() {
  ...
  fetch('https://sb-film.skillbox.cc/ping', { method: 'POST' })
    .then(() => {
      ...
      if (duration > 500) {
        showNetworkNotification('slow', 'Медленное соединение');
      } else {
        hideNetworkNotification();
      }
    })
    .catch(() => {
      showNetworkNotification('error', 'Проблема с сетью');
    });
}

// --- Инициализация мониторинга сети ---
function initNetworkMonitoring() {
  checkNetworkConnection(); // сразу проверим
